/* ======================================================
   ...
   Interactive Breeds Data, Login Auth, Django Backend API Binds
   ====================================================== */

// 1. CAT BREEDS DATABASE (Fallback / Seed data)
const catBreeds = [
    {
        id: "persian",
        name: "Persian Cat",
...
let searchQuery = '';
let currentUser = localStorage.getItem('meow_user_name') || '';
...
const noteRecipientName = document.getElementById('noteRecipientName');
const noteGreetingName = document.getElementById('noteGreetingName');
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    togglePasscodeBtn.addEventListener('click', () => {
        const type = loginPasscode.getAttribute('type') === 'password' ? 'text' : 'password';
        loginPasscode.setAttribute('type', type);
        togglePasscodeBtn.innerHTML = type === 'password' ? '<i class="fa-solid fa-eye"></i>' : '<i class="fa-solid fa-eye-slash"></i>';
    });

    userBadgeBtn.addEventListener('click', () => {
        if (currentUser) {
            if (confirm(`Logged in as "${currentUser}". Do you want to switch user or lock?`)) {
                localStorage.removeItem('meow_user_name');
                localStorage.removeItem('meow_logged_in');
                currentUser = '';
                checkLoginState();
            }
        } else {
...
// FETCH BREEDS FROM DJANGO BACKEND API IF AVAILABLE
async function fetchBreedsFromDjango() {
    try {
        ...
        console.log('Using static cat breeds database.');
    }
    renderBreeds();
}
...
function checkLoginState() {
    const isLoggedIn = localStorage.getItem('meow_logged_in') === 'true';
    if (isLoggedIn && currentUser) {
        loginOverlay.classList.add('hidden');
        userBadgeText.textContent = currentUser;
        
        // Personalize Site Text
        if (heroWelcomeName) heroWelcomeName.textContent = currentUser;
        if (noteRecipientName) noteRecipientName.textContent = currentUser;
        if (noteGreetingName) noteGreetingName.textContent = currentUser;
    } else {
        loginOverlay.classList.remove('hidden');
    }
}
...
async function handleLogin(e) {
    ...
    const passVal = loginPasscode.value.trim().toLowerCase();

    // Accept 'meow123', 'love', 'cat', or any non-empty passcode for friendly UX
    if (passVal === 'meow123' || passVal === 'love' || passVal === 'cat' || passVal.length >= 3) {
        currentUser = nameVal || 'Cat Lover';
        localStorage.setItem('meow_logged_in', 'true');
        localStorage.setItem('meow_user_name', currentUser);

        // Send Login Session Log to Django Backend
        try {
            await fetch('/api/login-log/', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ name: currentUser, passcode: passVal })
            });
        } catch (err) {
            console.log('Login logged locally.');
        }

        loginOverlay.classList.add('hidden');
        triggerConfettiHearts();
        showToast(`Welcome back to Meow World, ${currentUser}! 👑💖`);
        checkLoginState();
    } else {
        loginCard.classList.add('shake');
        setTimeout(() => loginCard.classList.remove('shake'), 500);
        showToast('Incorrect Passcode! Hint: try "meow123" or "love" 🐾');
    }
}
    ...
    filtered.forEach(breed => {
        ...
        card.innerHTML = `
            <div class="card-img-wrapper" onclick="openBreedModal('${breed.id}')">
                ...
                <span class="card-tag">${breed.tags[0] || 'Cute'}</span>
            </div>
            <div class="card-body">
...
// ADOPTION FORM SUBMISSION -> SENDS DIRECT POST TO DJANGO BACKEND!
async function handleAdoptionSubmit(e) {
    ...
    try {
        const response = await fetch('/api/adopt/', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ name, phone, email, breed, message })
        });

        const resData = await response.json();

        if (response.ok && resData.status === 'success') {
            showToast(`🎉 Thank you ${name}! Your inquiry for ${breed} is saved in Django Database! We will call you at ${phone}! 🐾`);
        } else {
            showToast(`Thank you ${name}! Inquiry recorded for ${breed}! 🐾`);
        }
    } catch (err) {
        showToast(`Thank you ${name}! Inquiry recorded for ${breed}! 🐾`);
    }
...
function startPurr() {
    ...
    purrOsc.frequency.setValueAtTime(55, audioCtx.currentTime);

    pulseOsc.frequency.setValueAtTime(24, audioCtx.currentTime);
    ...
    purrGain.gain.setValueAtTime(0.08, audioCtx.currentTime);
...
// HEART CONFETTI FOR LOVE NOTE / LOGIN SUCCESS
